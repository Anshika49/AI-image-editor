...
//   const fabricRef = useRef(null); // local Fabric instance
...
//     FabricImage.fromURL(project.currentImageUrl, { crossOrigin: "anonymous" }).then(img => {
//       canvas.add(img);
//       canvas.requestRenderAll();
//     });
//   }
...
//       clearTimeout(saveTimeout);
//       saveTimeout = setTimeout(() => {
//         if (!fabricRef.current || !project) return;
//         const canvasJSON = fabricRef.current.toJSON();
//         updateProject({ projectId: project._id, canvasState: canvasJSON });
//       }, 2000);
//     };
...
//     };
//   }, [fabricRef.current, onToolChange]);
...
// export default CanvasEditor;
 ...
 // useEffect(() => {
  ...
  //     fabricRef.current = canvas; // save local reference
  //     setCanvasEditor(canvas); // update context if needed
  ...
  //     const scaleFactor = window.devicePixelRatio || 1;
  //     if (scaleFactor > 1) {
  //       const elem = canvas.getElement();
  //       elem.width = project.width * scaleFactor;
  //       elem.height = project.height * scaleFactor;
  //       canvas.getContext().scale(scaleFactor, scaleFactor);
  //     }
